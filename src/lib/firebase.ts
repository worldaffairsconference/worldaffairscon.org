...
if (mode === "dev" || mode === "test") {
	const [firestoreHostname, firestorePort] =
		PUBLIC_FIREBASE_FIRESTORE_EMULATOR_HOST.split(":");
	...
	try {
		...
		connectAuthEmulator(
			auth,
			`http://${PUBLIC_FIREBASE_AUTH_EMULATOR_HOST}`,
			{
		...
		if (
			...
			!/already been started/u.test(error.message)
		)
			throw error;
	}
}
...
>(path: string, schema: T, parsingSchema: U) {
	...
	const { subscribe } = readable<
		(z.infer<T> & { invalid: false | z.ZodError }) | null
	>(null, (set) =>
		browser
...
export const userData: Readable<User | null> = derived(user, ($user, set) => {
	if ($user)
		return docStore(`users/${$user.uid}`, User, UserParse).subscribe(set);
	else set(null);
});
