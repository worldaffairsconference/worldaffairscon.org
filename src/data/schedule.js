const Schedule = [
  {
    name: 'Check-in',
    events: [
      {
        event: 'Doors Open for Delegate Check-in',
        speaker: '',
        timeOrRecording: '8:30 AM',
      },
      {
        event: 'Final In-person Check-in Time',
        speaker: '',
        timeOrRecording: '9:45 AM',
      },
    ],
  },
  {
    name: 'Opening Address & Keynote',
    events: [
      {
        event: 'Opening Remarks',
        speaker: '',
        timeOrRecording: '9:30 AM - 9:40 AM',
      },
      {
        event: 'Opening Keynote & QnA Session',
        speaker: 'C.K. Hoffler',
        timeOrRecording: '9:40 AM - 10:30 AM',
      },
    ],
  },
  {
    name: 'Plenary Session 1',
    events: [
      {
        event: 'Plenary Session',
        speaker: 'Joe Toscano & Tazin Khan',
        timeOrRecording: '10:40 AM - 11:20 AM',
      },
      {
        event: 'Plenary Session',
        speaker: 'Ismael Loutfi',
        timeOrRecording: '10:40 AM - 11:20 AM',
      },
      {
        event: 'Plenary Session',
        speaker: 'Leslie Madden',
        timeOrRecording: '10:40 AM - 11:20 AM',
      },
    ],
  },
  {
    name: 'Plenary Session 2',
    events: [
      {
        event: 'Plenary Session',
        speaker: 'Meghan Roach',
        timeOrRecording: '11:35 AM - 12:15 PM',
      },
      {
        event: 'Plenary Session',
        speaker: 'Rahul Singh',
        timeOrRecording: '11:35 AM - 12:15 PM',
      },
      {
        event: 'Plenary Session',
        speaker: 'Adrienne Batra',
        timeOrRecording: '11:35 AM - 12:15 PM',
      },
      {
        event: 'Plenary Session',
        speaker: 'Katherine Tattum',
        timeOrRecording: '11:35 AM - 12:15 PM',
      },
    ],
  },
  {
    name: 'Lunch & Networking',
    events: [
      {
        event: 'Lunch Break',
        speaker: '',
        timeOrRecording: '12:25 PM - 1:25 PM',
      },
      {
        event: 'Delegate Networking',
        speaker: '',
        timeOrRecording: '12:25 PM - 1:25 PM',
      },
    ],
  },
  {
    name: 'Plenary Session 3',
    events: [
      {
        event: 'Plenary Session',
        speaker: 'Matthew Ball',
        timeOrRecording: '1:35 PM - 2:15 PM',
      },
      {
        event: 'Plenary Session',
        speaker: 'Francesca Quinn',
        timeOrRecording: '1:35 PM - 2:15 PM',
      },
      {
        event: 'Plenary Session',
        speaker: 'Barry McInerney',
        timeOrRecording: '1:35 PM - 2:15 PM',
      },
      {
        event: 'Plenary Session',
        speaker: 'Pooja Handa',
        timeOrRecording: '1:35 PM - 2:15 PM',
      },
    ],
  },
  {
    name: 'Closing Address & Keynote',
    events: [
      {
        event: 'Keynote Address',
        speaker: 'Martin Luther King III',
        timeOrRecording: '2:25 PM - 3:25 PM',
      },
      {
        event: 'Closing Remarks',
        speaker: '',
        timeOrRecording: '3:25 PM - 3:40 PM',
      },
    ],
  },
];
export default Schedule;
