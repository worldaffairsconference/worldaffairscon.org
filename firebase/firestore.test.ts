...
const testEnv = await initializeTestEnvironment({
	projectId: JSON.parse((await fs.readFile('.firebaserc')).toString()).projects.test,
	firestore: {
		rules: await fs.readFile('firebase/firestore.rules', 'utf8'),
		host: '127.0.0.1',
		port: JSON.parse((await fs.readFile('firebase.json')).toString()).emulators.firestore.port
	}
});
